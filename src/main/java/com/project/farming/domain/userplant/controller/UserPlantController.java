...
public class UserPlantController {
    ...
    @Operation(summary = "사용자 식물 목록 조회",
            description = """
                    ...
                    일부 정보만 반환합니다.
                    (userPlantId, plantName(식물 이름), plantNickname(식물 별명),
    ...
    @Operation(summary = "특정 사용자 식물 정보 수정",
            description = """
                    ...
                    1. 기본 식물 12종 외의 사용자 입력 식물이라면 식물 이름(plantName)을 수정할 수 있습니다.
                    2. 식물의 별명(plantNickname), 메모(notes), 심은 장소(plantingPlace, 옮겨 심는 경우),
                       3가지 상태(물 주기(watered), 가지치기(pruned), 영양제 주기(fertilized)), 식물 이미지(file)를 수정할 수 있습니다.
